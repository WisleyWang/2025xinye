...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def mixup_partial(self, img1,lb1, alpha=0.4):
        ...
        if random.random()<0.5:
            img1[l_eye[0]: l_eye[1], l_eye[2]:l_eye[3]] = lam *img1[l_eye[0]: l_eye[1], l_eye[2]:l_eye[3]] + \
                                    (1 - lam)*image2[l_eye[0]: l_eye[1], l_eye[2]:l_eye[3]]
        img1[r_eye[0]: r_eye[1], r_eye[2]:r_eye[3]] = lam *img1[r_eye[0]: r_eye[1], r_eye[2]:r_eye[3]] + \
                                (1 - lam)*image2[r_eye[0]: r_eye[1], r_eye[2]:r_eye[3]]
        if random.random()<0.35:
            img1[mouth[0]: mouth[1], mouth[2]:mouth[3]] = lam *img1[mouth[0]: mouth[1], mouth[2]:mouth[3]] + \
    ...
    def init_data_aug_method(self):
        trans = A.Compose([           
            ...
            ], p = 0), # 0 if self.config['with_landmark'] else  1
            A.RandomScale(scale_limit=(-0.5, 0), p=0.3),  # 缩小图片至原始尺寸的50%-100%
            A.PadIfNeeded(
                    min_height=self.config['resolution'],
                    min_width=self.config['resolution'],
                    border_mode=cv2.BORDER_REPLICATE,  # 拉伸边缘（或用BORDER_WRAP）
                    p=1
                        ),
            A.OneOf([
            ...
            A.ImageCompression(quality_lower=self.config['data_aug']['quality_lower'], quality_upper=self.config['data_aug']['quality_upper'], p=1),
           A.Resize(height=self.config['resolution'], width=self.config['resolution'], p=1) ], 
