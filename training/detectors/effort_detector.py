...
class EffortDetector(nn.Module):
    ...
    def compute_weight_loss(self):
        weight_sum_dict = {}
        num_weight_dict = {}
        orthogonal_losses = 0.0
        cnt = 0
        for name, module in self.backbone.named_modules():
            if isinstance(module, SVDResidualLinear):
                cnt += 1
                orthogonal_losses+=module.compute_orthogonal_loss()
        ...
        for k in weight_sum_dict.keys():
            ...
            loss2 += torch.sqrt(torch.mean(S_sum**2))
        loss2 /= len(weight_sum_dict.keys())
        if cnt!=0:
            loss1 = orthogonal_losses/cnt
        else:
            loss1 = 0.0
        return loss1, loss2
    ...
    def get_losses(self, data_dict: dict, pred_dict: dict) -> dict:
        ...
        # loss1, loss2 = self.compute_weight_loss()
...
@DETECTOR.register_module(module_name='effort2')
class EffortDetector2(nn.Module):
    def __init__(self, config):
        super(EffortDetector2, self).__init__()
        self.config = config
        self.backbone = self.build_backbone(config)
        # self.head = nn.Linear(1024, 2)
        self.loss_func = nn.CrossEntropyLoss()
        ...
        if config['pretrained']:
            ...
            # state_dict = {k.replace('visual.','backbone.'):v for k, v in state_dict.items() if 'visual' in k}
    ...
    def build_backbone(self, config):
        ...
        model = create_model(config['backbone_name'], 
                            pretrained=True, num_classes=2)
        # print("开始替换")
        # model = replace_attention_in_vit(model)
        # Apply SVD to self_attn layers only
        # ViT-L/14 224*224: 1024-1
        model = apply_svd_residual_to_self_attn(model, r=1024-1, attn_name='attn')

        for param_name, param in model.named_parameters():
            if any(x in param_name for x in ['head']):
        ...
        return model
    def features(self, data_dict: dict) -> torch.tensor:
        feat = self.backbone.forward_features(data_dict['image'])
        return feat
...
def apply_svd_residual_to_self_attn(model, r,attn_name='self_attn'):
    for name, module in model.named_children():
        if attn_name in name :
            ...
            apply_svd_residual_to_self_attn(module, r,attn_name)
