...
class SBIDataset(DeepfakeAbstractBaseDataset):
    def __init__(self, config=None, mode='train'):
        ...
        self.real =  [(img, label) for img, label in zip(self.image_list, self.label_list) if label == 0]

        # self.real =  [(img, label) for img, label in zip(self.image_list, self.label_list)]
        self.real_imglist = []
        for p,lb in self.real:
            img_path = os.path.join(self.config["rgb_dir"], p)
            ld_p = img_path.replace('_crop', '_crop_landmarks').replace('.jpg', '.npy')
            # print(ld_p)
            if os.path.exists(ld_p):
                self.real_imglist.append((img_path,ld_p, lb))
        print("Real image number: ",len(self.real_imglist))
        # Init SBI
    ...
    def __getitem__(self, index):
        ...
        real_image_path,real_landmark_path, real_label = self.real_imglist[index]
        ...
        try:
            landmark = np.load(real_landmark_path)
        except:
            landmark=None
        h_orig, w_orig = real_image.shape[:2]
        real_image = cv2.resize(real_image, (self.size, self.size), interpolation=cv2.INTER_CUBIC)

        scale_x = self.size / w_orig
        scale_y = self.size / h_orig
        ...
        if landmark is not None and (scale_x != 1 or scale_y != 1):
        ...
        if fake_image is None:
            ...
            fake_label = real_label # 0
        else:
            fake_label = 1
        ## aug
        if self.config['use_data_augmentation']:
            kwargs = {'image': fake_image}
            transformed = self.transform(**kwargs)
            fake_image = transformed['image']
            kwargs = {'image': real_image}
            transformed = self.transform(**kwargs)
            real_image = transformed['image']
        ...
        return {"fake": (fake_image_trans, fake_label), 
                "real": (real_image_trans, real_label), "image_path":real_image_path}
    ...
    def collate_fn(batch):
        ...
        image_path += [ p.split('.')[0]+'_fake.png' for p in image_path]
