...
def load_state_dict_from_hf(model_id: str, filename: str = HF_WEIGHTS_NAME):
    assert has_hf_hub(True)
    hf_model_id, hf_revision = hf_split(model_id)
    if 'large_patch14' in model_id:
        model_id = 'timm/vit_base_patch16_224.augreg_in1k'
        cached_safe_file = os.path.expanduser(f"~/.cache/huggingface/hub/{_HG_MODEL2_}/blobs")
        if os.path.exists(cached_safe_file):
            name = os.listdir(cached_safe_file)[0]
            try:
                safe = safetensors.torch.load_file(os.path.join(cached_safe_file,name))
            except:
                safe = torch.load(os.path.join(cached_safe_file,name), map_location='cpu')
            return safe      
    ...
    if _has_safetensors:
        for safe_filename in _get_safe_alternatives(filename):
            try: 
                ...
                safe = safetensors.torch.load_file(cached_safe_file, device="cpu")
                hf_model_id = 'timm/vit_base_patch16_224.augreg_in1k'
                hf_hub_download(repo_id=hf_model_id, filename=safe_filename, revision=hf_revision)
                change_repo_id()
                return safe
    ...
    safe = torch.load(cached_file, map_location='cpu')
    hf_model_id = 'timm/vit_base_patch16_224.augreg_in1k'            
    hf_hub_download(repo_id=hf_model_id, filename=filename, revision=hf_revision) 
    change_repo_id()
...
def change_repo_id():
    name1 = os.listdir(os.path.expanduser(f"~/.cache/huggingface/hub/{_HG_MODEL1_}/blobs"))[0]
    name2 = os.listdir(os.path.expanduser(f"~/.cache/huggingface/hub/{_HG_MODEL2_}/blobs"))[0]
    os.system(f'mv ~/.cache/huggingface/hub/{_HG_MODEL1_}/blobs/{name1} '\
                          + f'~/.cache/huggingface/hub/{_HG_MODEL2_}/blobs/{name2}')
    os.system(f'rm -rf ~/.cache/huggingface/hub/{_HG_MODEL1_}/')
    return None
...
_HG_MODEL1_="models--timm--vit_large_patch14_clip_224.openai"
_HG_MODEL2_ = "models--timm--vit_base_patch16_224.augreg_in1k"
