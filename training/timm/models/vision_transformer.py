...
class Attention(nn.Module):
    ...
    def split_qkv_weights(self,):
        q_weight, k_weight, v_weight = self.qkv.weight.data.chunk(3, dim=0)
        self.q = nn.Linear(self.qkv.in_features, self.qkv.out_features // 3, bias=self.qkv.bias is not None)
        self.k = nn.Linear(self.qkv.in_features, self.qkv.out_features // 3, bias=self.qkv.bias is not None)
        self.v = nn.Linear(self.qkv.in_features, self.qkv.out_features // 3, bias=self.qkv.bias is not None)
        self.q.weight.data = q_weight
        self.k.weight.data = k_weight
        self.v.weight.data = v_weight
        if self.qkv.bias is not None:
            self.q.bias.data = self.qkv.bias.data[:self.qkv.out_features // 3]
            self.k.bias.data = self.qkv.bias.data[self.qkv.out_features // 3: 2 * self.qkv.out_features // 3]
            self.v.bias.data = self.qkv.bias.data[2 * self.qkv.out_features // 3:]
        del self.qkv
